...
| **P4 A trap I actually hit** *(one gotcha, three ways)* | Split: the symptom · the cause · the gate | Tell a **real** war story from your own bring-up (pick the one that's truly yours — candidates: the **editor/cgroup reaping** trap — you start the server from a VS Code terminal and the editor silently reaps the whole process tree, no error, it just vanishes; or the **embeddings dim mismatch** silently corrupting RAG retrieval). Three ways it shows up: the **symptom** you saw, the **cause** once you dug in, and **this repo: a gate that STOPS the run** before it costs you a day. *(Describe the experience, never a tok/s number.)* | The symptom → cause → gate-that-STOPS motif lands on a real story |
