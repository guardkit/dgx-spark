...
| Router escalates across matrix sets | Every escalation becomes an evict-and-reload on the shared 128 GB pool; cold loads of 110 s–6 min are documented; concurrent sets can approach the 115 GB ceiling | Every model named in a routing policy's target set is co-resident in one declared matrix set — **or** the policy is sticky-per-task and re-routes only at task boundaries | DF-031 (ai-transition `docs/decisions/`) |
| Self-improvement notebook treated as procedure | The loop compounds whatever works, not what you meant (the Factorio admin-console case: it learned to cheat, then learned to cheat *better*). Unpromoted lessons become a shadow procedure that contradicts the pinned one | The notebook is read as a drift report: its diff is reviewed, real lessons are promoted to a registry row by PR (§6), and the loop is **OFF** for any runbook-execution session | DF-031 (ai-transition `docs/decisions/`) |
