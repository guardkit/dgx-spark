...
3. **The consequence** — so you don't stack for speed. You stack for **capacity** (run the model that *cannot exist* on one 128 GB box — DeepSeek-V4-Flash-0731's native weights are ~167 GB) and **parallel throughput**, then **time-share** the boxes: the swap fleets own them day-to-day, the one oversized strategist takes both for a session, and gated drain/revive makes the handover provable.
...
- **`./RUNBOOK-deepseek-v4-flash-0731-strategist.md`** — the strategist overlay P3 films (supersedes the bring-up runbook's old Phase 8 jasl lane, which is now the pinned fallback). Its PINS block is the source of every number and flag in P3.
- `two-spark-serving-research-and-references.md` (the gotchas + the expected numbers)
- **`./RUNBOOK-orbit-globe-demo-capture.md`** — the payoff segment this video tees at P5 (a MacBook agent doing real work against the strategist).
