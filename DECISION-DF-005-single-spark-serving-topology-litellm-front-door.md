...
- [x] **PINS:** added a LiteLLM row (`litellm[proxy]`, record installed version) and its port (`4000`), plus a `GB10_CORES 20` pin for the CPUAffinity ranges. Recon checks the LiteLLM release the morning of a run (`github.com/BerriAI/litellm`), same as the other pins.
