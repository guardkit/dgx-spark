...
> **Addendum — second sweep at execution time (2026-08-08 ~17:10, run session).** Pin checks re-run live: recipe HEAD still bf3d4ea (docs-only, 1 ahead), weights revision 7872f01b unchanged and staged complete on both nodes (48 shards, 156 GiB deref = the 167 decimal GB pin), image absent both → Phase 2 builds. Independent source re-sweep found no new pin-affecting items beyond this report; tail movement only: t/372268 now ends ~#675 (08-08; #665 spurious-"arguments" strip tool circulating, unvalidated — watch stance unchanged), t/378824 shows post-08-04 traffic incl. a positive two-Spark field deploy. Item 1 below (d728faee leftovers) is DONE — the executing runbook copy reads cd366d5e at both sites (commit a1a6237/72e7b71 lineage). In-run guidance items 2 adopted verbatim for Phases 4–7. Run proceeds on PINS.
