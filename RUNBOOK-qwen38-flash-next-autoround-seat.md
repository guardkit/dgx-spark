...
**Status:** **`19g` configuration retained on 2026-09-10 under the user-selected 120 decimal GB total-RAM ceiling.** Normal endpoint: `http://spark-fcf6.local:8888/v1`, development APIs disabled. Protocol, fixed-prefix 250K, growing-history 20K/96K/250K and sustained-generation checks passed; 8192 tokens measured 58.79 tok/s decode over 139.58 seconds. Peak effective used RAM was 118.534 GB. A `20g` trial crossed 120 GB and was stopped; its additional two-context warm-reuse test also failed. See [current allocation results](./RESULTS-qwen38-flash-next-kv-ceiling-20260910T181629Z.md) and the [prior 12 GB validation](./RESULTS-qwen38-flash-next-autoround-seat-20260910T165917Z.md). No Factory task-quality comparison or multi-day soak test has run.
...
The author's roughly 50–60 tok/s results use different conditions, including a remote-RAM PLE table, thinking enabled and a different top-k path. The local `12g` and `19g` configurations have their own serving receipts; these are not reproductions of those external conditions. Exact top-k trades prefill speed for avoiding the custom kernel's reported long-context failure. The optional [Factory evaluation](./QWEN38-software-factory-evaluation.md) still needs to rank configurations by time to correctly completed tasks.
...
CHUNK=8192
GMU=0.01
KV_BYTES=19g                       # selected from the user-authorized 120 GB ceiling trial
MTP=3
EXACT_TOPK=1
DET_TOPK=0
