...
## Addendum, 2026-09-03 — the product-owner adapter has now been measured intact

The follow-up (`RESULTS-vllm-adapter-followup-2026-09-02.md`, beside this file) served all four corrected exports together. Every adapter loaded its expert blocks in all thirty layers. With vLLM's deterministic-kernel mode **off**, the product-owner adapters scored 17, 16, 16 (po-v6) and 17, 17, 17 (po-v5) against 16, 16, 16 for the merged po-v6 weights on the same engine, and the coach scored 6 of 6 again. With the mode **on**, the same po-v6 export ran away to the 16,384-token ceiling three times, so that mode is not safe for long generation on this build and the repeatability it bought above comes at that price. The "never served with its expert half loaded" status of the product owner in this document is therefore closed. Read the follow-up for the receipts, the cache and slot figures at dial 0.65, and the conditions a seat move still needs.
