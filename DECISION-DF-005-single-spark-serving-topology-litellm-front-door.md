...
- [x] **PINS:** added a pinned LiteLLM row (`litellm[proxy]==1.89.4`, validated on GB10 — wheels-only ~16s install) and its port (`4000`), plus a `GB10_CORES 20` pin for the CPUAffinity ranges. Recon checks the LiteLLM release the morning of a run (`github.com/BerriAI/litellm`), same as the other pins.
