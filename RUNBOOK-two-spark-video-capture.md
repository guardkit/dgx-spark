...
3. **The consequence** — so you don't stack for speed. You stack for **capacity** (run the model that won't fit 128 GB) and **parallel throughput**, then **time-share** the boxes: swap the fleet on one node, reserve TP for the one oversized strategist.
...
- **`./RUNBOOK-two-spark-bring-up.md`** — the executable, gated arc this video *films* (P2 = its Phases 2–6, P3 = its Phase 9). Run it once before recording.
...
| **P4 Payoff** *(architecture)* | The two SVG diagrams | Walk the layered stack: one **LiteLLM :4000** front door → **llama-swap** pool (fleet + always-on nomic) on Node A → **vLLM TP=2** strategist across both nodes → Postgres+pgvector on the NAS. State the **memory rule** (the big strategist and a full swap pool can't co-reside — choose per session) and the **no auto cloud fallback** guard (DF-001). | Topology + "share by time" rule explained |
