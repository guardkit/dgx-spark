...
The retained normal endpoint is `http://spark-fcf6.local:8888/v1`, model `qwen3.8-flash-next-autoround`, now using **19 GB KV under a 120 decimal GB total-RAM guard**. The [allocation trial](./RESULTS-qwen38-flash-next-kv-ceiling-20260910T181629Z.md) measured 58.79 tok/s on a capped code stream and passed protocol, single-prefix 250K and growing-history checks. The 20 GB profile crossed the memory ceiling and did not demonstrate fast reuse of two independent full 250K contexts; do not assume its advertised cache-token capacity translates into that behavior. The [earlier 12 GB validation](./RESULTS-qwen38-flash-next-autoround-seat-20260910T165917Z.md) remains separate. These are serving receipts, not task-quality results: the module hit its output cap, and no Factory task or baseline comparison ran. Freeze the actual selected KV allocation and memory policy with the evaluation configuration.
