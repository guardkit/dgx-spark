...
## Correction, 2026-09-03 afternoon: Rich's headroom evidence changes the order of things

Rich reports that when the boxes ran DeepSeek V4 Flash they sat at about 114 to 120 GB of the 121 GB pool and held stable for three to four days, with the processor turned down, and that the recent NVIDIA fixes have helped stability a lot. That is direct evidence the safe operating band goes much closer to full than this note assumed.

Re-reading our own dial measurements against that band: at dial 0.60 the 16-bit adapter process beside the resident coding model left 4.2 GB free with 269,516 tokens of cache, so total use was about 116.8 GB, inside Rich's observed stable band. The eight-sentence campaign needs about 131,000 tokens of cache (four concurrent 32,768-token requests). So the 16-bit path already covers the campaign at dial 0.60, provided the start order rule is kept (adapter process first, coding model second; the reverse order is what the kernel killed on 2026-09-03). Two of the earlier statements in this note are therefore too strong: "the memory budget did not close" should read "the budget closes at dial 0.60 with about 4 GB to spare, which Rich's experience says is workable", and FP8 is a speed and margin lever, not a precondition for the campaign.

What this changes in the recommendation: the campaign can be run on 16-bit weights now. FP8 is still worth measuring, for the forum's 1.7 times decode speed and for a comfortable margin, and can be measured alongside rather than before. NVFP4 stays a later experiment for the reasons above. Not yet known: whether "processor turned down" (a lower clock or power setting) is part of why those long runs held, and whether the same setting should be on for the campaign; that is Rich's call and a note for the run card.
