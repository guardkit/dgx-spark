...
*(**NVIDIA Sync scorecard** — added 2026-08-12, after NVIDIA's [Cluster Assistant walkthrough](https://www.youtube.com/watch?v=MehBUQtb9qM). Sync automates the Phase-3 CX-7 network creation and **all of Phase 5** — 2 of the 14 gates above, and none of the load-bearing ones. Against the 12 findings below it would have **detected 2 and fixed 0**: **#1** via its bandwidth/latency warning — the 2.84 GB/s power-capped link would have tripped it, and that one cost hours of diagnosis, so this is a real saving — and **#9** via its physical-connection check. **#2 and #4 are unknown**: both depend on what Sync writes to netplan/NM, untested here. The other 8 sit past NVIDIA's own stated boundary — *"Sync doesn't yet set up such workloads."* See the Sync note in [RUNBOOK Phase 3](./RUNBOOK-two-spark-bring-up.md).)*
