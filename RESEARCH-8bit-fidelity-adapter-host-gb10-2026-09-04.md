...
## Dated addendum, 2026-09-05 ~00:25Z — candidate A was tried; the verdict is measured

Everything below is coach-verified (receipts under `~/fine-tuning/output/int8-offline-checkpoint-2026-09-04/`).

- **Load-time INT8 (`--quantization int8_per_channel_weight_only`) cannot be used on this box.** Its loader ate the whole 121 GB pool while still loading experts and the kernel killed three processes before the trial could be stopped (21:03–21:09Z, 4 September). Never again on the production entry; separate container plus memory watchdog is now the rule.
- **An offline compressed-tensors INT8 weight-only checkpoint of the experts (pack-quantized) is the working route.** Streaming quantisation takes about 100 seconds on the CPU with a working set of one tensor. vLLM 0.25.0 routes it to the **Marlin** WNA16 expert kernel, which carries the adapter hook; there is no Triton kernel for weight-only integer experts, and the production flag `--moe-backend triton` is fatal on that path.
- **Two vLLM 0.25.0 defects met on the way, neither filed upstream at Rich's decision.** (1) During weight loading the allocator is told to hand out 20 MB blocks and never split them, and the Marlin layout forces a 2 MB scratch copy per expert, so 256 blocks of 20 MB are orphaned per layer: about 150 GB of waste for this model, which a discrete GPU recycles when video memory runs out and a unified-memory box cannot. `PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True` fixes it outright: 7 shards in 53 s, box flat at ~67 GB free. (2) With per-channel scales the Marlin weight preparation crashes (`marlin_utils.py:343`, group size unset). Scales per group of 64 avoid it on the same path.
- **The group-64 checkpoint served with all six adapters:** 28 GB, loaded in 48 s, ready in about four minutes, peak 54 GB used.
- **Fidelity and speed, against the FP8 host in production and the 16-bit adapter:** on the live prompt that failed sentence eight it answered correctly 3 of 3 (FP8 misreads 3 of 3; 16-bit 0 of 3), so the FP8 base is the odd one out, confirmed a second way. In the coach exam it passed 4 of 6 runs against 6 of 6 for FP8 and for 16-bit, both losses a defective bundle approved. Product-owner exam 16, 17, 17 of 17 with no runaway (at or above FP8). Speed 20 (coach) and 21 (PO) tokens per second against FP8's 25 and 28: the Marlin kernel runs at 16-bit speed on this chip.
- **Verdict:** not a replacement for the FP8 host. It removes one misreading the deterministic guard already voids, and costs two held catches and about a fifth of the speed. Rich's option one stands. The third held-out coach case was not measured on any base and should be, using the purpose-built runner from 4 September, before anything further is concluded about the integer form.
