...
**Status:** ACCEPTED — **implemented 2026-06-24** in the single-Spark runbook's Phase 5.4, then **extracted 2026-06-29** into a standalone additive overlay, [`RUNBOOK-litellm-front-door.md`](./RUNBOOK-litellm-front-door.md) (install + the three gates; public config at `examples/litellm-config.public.yaml`; gates recorded in its Phase 5 Decision-Gate table). The base runbook [`RUNBOOK-single-spark-bring-up.md`](./RUNBOOK-single-spark-bring-up.md) now stands up the llama-swap `:9000` fleet alone; the front door is the optional overlay run on top — the base+overlay pattern in [`RUNBOOK-CONVENTIONS.md`](./RUNBOOK-CONVENTIONS.md) §2.1 (a fleet-only operator skips it; per-project token tracking is the reason to add it). The only step left is the operator's pre-filming end-to-end dry-run (§3 last item).
**Date:** 2026-06-24
...
The LiteLLM phase was added to `RUNBOOK-single-spark-bring-up.md` (Phase 5.4) 2026-06-24, then **extracted 2026-06-29** into the standalone overlay [`RUNBOOK-litellm-front-door.md`](./RUNBOOK-litellm-front-door.md) (Phase 1 asserts the base fleet is green on `:9000`; Phases 2–4 install + deploy + the three gates; Phase 5 is its Decision Gate) — following the repo's recon → pinned-steps → inline-gates convention. The phase references in the boxes below now map to that overlay.
...
- [x] **Install + run LiteLLM** as a **user** systemd unit (mirror the llama-swap supervision model: `systemctl --user`, `loginctl enable-linger`; never from a VS Code terminal — see the v3 §10.2 cgroup trap). Overlay Phases 2–3.
...
- [x] **▶ GATE — no cloud fallback (DF-001):** asserts `fallbacks: []` **and** `context_window_fallbacks: []` (anchored greps) **and** — after stripping `#` comments — no `claude-*`/cloud model in a fallback chain. FAIL → STOP. This is the on-camera "the one community feature I deliberately disable" beat. Overlay Phase 4.1.
- [x] **▶ GATE — CPU-pin LiteLLM separately from llama-swap (WARN):** under concurrent multi-model load, LiteLLM and llama-swap sharing a core can yield LiteLLM 504s + flaky llama-swap health checks. Non-overlapping `CPUAffinity=` on the two user units, asserted **disjoint** — **WARN on overlap, don't STOP** (the disjointness check is sound and self-verifying; the 504s rationale is community-sourced, not authoritative — see the verification note). Ranges for the **20-core** GB10 CPU (10× Cortex-X925 + 10× Cortex-A725) — *not* a 72-core Grace. Overlay Phase 4.2.
...
- **DECISION-DF-031** (`ai-transition/docs/decisions/DECISION-DF-031-task-aware-routing-two-routers-split-by-attendance.md`, 2026-08-15) — the stage-three successor this decision's §1.1 anticipated: task-aware routing ruled as two routers split by attendance (attended harness router under DF-003 · unattended Jarvis router under DF-001), sharing sticky-per-task.
