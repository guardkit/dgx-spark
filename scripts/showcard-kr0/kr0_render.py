#!/usr/bin/env python3
"""KR-0: one headless FLUX.1-Kontext subject-conditioned render.

Extends the CR-0 render gate (cr0_render.py) with the one code addition DF-024
calls for: a reference-image input slot. The subject photo is uploaded to
ComfyUI via POST /upload/image (multipart), its returned name is injected into
the SC_SUBJECT_IMAGE (LoadImage) slot, and the rest of the SC_* discipline is
unchanged: loud field-preexistence injection, submit /prompt, poll /history,
download /view, gate EXACT output size, write a JSON receipt.

Kontext conditions the subject via ReferenceLatent in the conditioning stream;
output size is owned by the SC_SIZE latent node (EmptySD3LatentImage), NOT by
the reference — so exact 1280x720 out is enforced here identically to CR-0.

Exit 0 = PASS (image at requested size). Nonzero = a gate failed.
"""
import argparse
import hashlib
import io
import json
import mimetypes
import struct
import sys
import time
import urllib.error
import urllib.parse
import urllib.request
import uuid
from pathlib import Path


def find(graph: dict, title: str):
    for nid, node in graph.items():
        if node.get("_meta", {}).get("title") == title:
            return nid, node
    return None, None


def inject(node: dict, title: str, field: str, value) -> bool:
    """Set inputs[field] only if the field already exists — loud otherwise."""
    if field not in node["inputs"]:
        print(f"FAIL: {title} node ({node['class_type']}) has no input "
              f"'{field}' — its inputs are {sorted(node['inputs'])}. "
              f"ComfyUI would silently ignore the injection; fix the slot.")
        return False
    node["inputs"][field] = value
    return True


def post_json(url: str, payload: dict) -> dict:
    req = urllib.request.Request(
        url, data=json.dumps(payload).encode(),
        headers={"Content-Type": "application/json"})
    try:
        with urllib.request.urlopen(req, timeout=30) as r:
            return json.loads(r.read())
    except urllib.error.HTTPError as e:
        body = e.read().decode(errors="replace")
        print(f"FAIL: POST {url} -> HTTP {e.code}\n{body[:2000]}")
        sys.exit(1)


def upload_image(server: str, path: Path) -> str:
    """POST /upload/image (multipart/form-data); return the stored name."""
    data = path.read_bytes()
    ctype = mimetypes.guess_type(path.name)[0] or "application/octet-stream"
    boundary = f"----kr0{uuid.uuid4().hex}"
    buf = io.BytesIO()

    def w(s):
        buf.write(s.encode() if isinstance(s, str) else s)

    w(f"--{boundary}\r\n")
    w(f'Content-Disposition: form-data; name="image"; filename="{path.name}"\r\n')
    w(f"Content-Type: {ctype}\r\n\r\n")
    w(data)
    w("\r\n")
    # overwrite so a re-run reuses the same server-side name deterministically
    w(f"--{boundary}\r\n")
    w('Content-Disposition: form-data; name="overwrite"\r\n\r\n')
    w("true\r\n")
    w(f"--{boundary}--\r\n")

    req = urllib.request.Request(
        f"{server}/upload/image", data=buf.getvalue(),
        headers={"Content-Type": f"multipart/form-data; boundary={boundary}"})
    try:
        with urllib.request.urlopen(req, timeout=60) as r:
            resp = json.loads(r.read())
    except urllib.error.HTTPError as e:
        body = e.read().decode(errors="replace")
        print(f"FAIL: POST /upload/image -> HTTP {e.code}\n{body[:2000]}")
        sys.exit(1)
    name = resp.get("name")
    if not name:
        print(f"FAIL: /upload/image returned no name: {json.dumps(resp)[:500]}")
        sys.exit(1)
    sub = resp.get("subfolder", "")
    stored = f"{sub}/{name}" if sub else name
    print(f"uploaded subject -> {stored} (type={resp.get('type','input')})")
    return stored


def png_size(data: bytes):
    if len(data) < 24 or data[:8] != b"\x89PNG\r\n\x1a\n":
        return None
    w, h = struct.unpack(">II", data[16:24])
    return w, h


def main() -> int:
    ap = argparse.ArgumentParser(description=__doc__)
    ap.add_argument("workflow", type=Path, help="SC_*-slotted Kontext workflow JSON")
    ap.add_argument("--server", default="http://127.0.0.1:8188")
    ap.add_argument("--subject", type=Path, required=True,
                    help="reference photo (the operator's face) to condition on")
    ap.add_argument("--prompt", required=True, help="the edit instruction")
    ap.add_argument("--seed", type=int, default=42)
    ap.add_argument("--width", type=int, default=1280)
    ap.add_argument("--height", type=int, default=720)
    ap.add_argument("--model", default=None,
                    help="override SC_CKPT model filename")
    ap.add_argument("--steps", type=int, default=None, help="override SC_STEPS")
    ap.add_argument("--guidance", type=float, default=None,
                    help="override SC_GUIDANCE (FluxGuidance)")
    ap.add_argument("--timeout", type=int, default=900, help="poll cap, seconds")
    ap.add_argument("-o", "--out", type=Path, required=True, help="output PNG")
    args = ap.parse_args()

    if not args.subject.exists():
        print(f"FAIL: subject photo not found: {args.subject}")
        return 1

    graph = json.loads(args.workflow.read_text())

    # --- upload the reference photo, wire it into SC_SUBJECT_IMAGE ---
    stored = upload_image(args.server, args.subject)
    _, subj = find(graph, "SC_SUBJECT_IMAGE")
    if subj is None:
        print("FAIL: no SC_SUBJECT_IMAGE slot (LoadImage) in the graph")
        return 1
    img_field = subj.get("_meta", {}).get("image_field", "image")
    if not inject(subj, "SC_SUBJECT_IMAGE", img_field, stored):
        return 1

    _, prompt_node = find(graph, "SC_PROMPT")
    if prompt_node is None:
        print("FAIL: no SC_PROMPT slot")
        return 1
    text_field = prompt_node.get("_meta", {}).get("text_field", "text")
    if not inject(prompt_node, "SC_PROMPT", text_field, args.prompt):
        return 1

    _, seed_node = find(graph, "SC_SEED")
    if seed_node is None:
        print("FAIL: no SC_SEED slot")
        return 1
    seed_field = seed_node.get("_meta", {}).get("seed_field", "seed")
    if not inject(seed_node, "SC_SEED", seed_field, args.seed):
        return 1

    _, size_node = find(graph, "SC_SIZE")
    if size_node is None:
        print("FAIL: no SC_SIZE slot")
        return 1
    if not (inject(size_node, "SC_SIZE", "width", args.width) and
            inject(size_node, "SC_SIZE", "height", args.height)):
        return 1

    if args.steps is not None:
        _, steps_node = find(graph, "SC_STEPS")
        if steps_node is None:
            print("FAIL: --steps given but no SC_STEPS slot")
            return 1
        if not inject(steps_node, "SC_STEPS", "steps", args.steps):
            return 1

    if args.guidance is not None:
        _, g = find(graph, "SC_GUIDANCE")
        if g is None:
            print("FAIL: --guidance given but no SC_GUIDANCE slot")
            return 1
        if not inject(g, "SC_GUIDANCE", "guidance", args.guidance):
            return 1

    if args.model:
        _, ckpt = find(graph, "SC_CKPT")
        if ckpt is None:
            print("FAIL: --model given but no SC_CKPT slot")
            return 1
        if "unet_name" in ckpt["inputs"]:
            ckpt["inputs"]["unet_name"] = args.model
        elif "ckpt_name" in ckpt["inputs"]:
            ckpt["inputs"]["ckpt_name"] = args.model
        else:
            print(f"FAIL: SC_CKPT ({ckpt['class_type']}) has neither "
                  f"unet_name nor ckpt_name")
            return 1

    t0 = time.monotonic()
    sub = post_json(f"{args.server}/prompt", {"prompt": graph, "client_id": "kr0"})
    pid = sub.get("prompt_id")
    if not pid:
        print(f"FAIL: no prompt_id: {json.dumps(sub)[:500]}")
        return 1
    print(f"submitted prompt_id={pid}")

    images, poll_errors = None, 0
    while time.monotonic() - t0 < args.timeout:
        time.sleep(2)
        try:
            with urllib.request.urlopen(f"{args.server}/history/{pid}",
                                        timeout=30) as r:
                hist = json.loads(r.read()).get(pid)
            poll_errors = 0
        except (urllib.error.URLError, OSError, json.JSONDecodeError) as e:
            poll_errors += 1
            print(f"  poll error ({poll_errors}/5): {e}")
            if poll_errors >= 5:
                print("FAIL: ComfyUI unreachable for 5 consecutive polls")
                return 1
            continue
        if not hist:
            continue
        status = hist.get("status", {})
        if status.get("status_str") == "error":
            print(f"FAIL: execution error: {json.dumps(status)[:2000]}")
            return 1
        for node_out in hist.get("outputs", {}).values():
            if node_out.get("images"):
                images = node_out["images"]
                break
        if images:
            break
        if status.get("completed") is True:
            print(f"FAIL: completed with no images: {json.dumps(hist)[:1000]}")
            return 1
    if not images:
        print(f"FAIL: no image within {args.timeout}s (render timeout gate)")
        return 1

    seconds = round(time.monotonic() - t0, 1)
    img = images[0]
    q = urllib.parse.urlencode({"filename": img["filename"],
                                "subfolder": img.get("subfolder", ""),
                                "type": img.get("type", "output")})
    try:
        with urllib.request.urlopen(f"{args.server}/view?{q}", timeout=60) as r:
            data = r.read()
    except (urllib.error.URLError, OSError) as e:
        print(f"FAIL: image download (/view?{q}) failed: {e}")
        return 1

    size = png_size(data)
    args.out.parent.mkdir(parents=True, exist_ok=True)
    args.out.write_bytes(data)
    if size != (args.width, args.height):
        print(f"FAIL: got {size}, wanted {(args.width, args.height)} "
              f"(saved to {args.out} for inspection)")
        return 1

    model_sha = None
    receipt = {
        "workflow": str(args.workflow),
        "subject": str(args.subject), "uploaded_as": stored,
        "prompt": args.prompt,
        "model_override": args.model, "steps_override": args.steps,
        "guidance_override": args.guidance,
        "seed": args.seed, "seed_field": seed_field, "text_field": text_field,
        "size": [args.width, args.height], "seconds": seconds,
        "prompt_id": pid, "sha256": hashlib.sha256(data).hexdigest(),
        "model_file_sha256": model_sha, "out": str(args.out),
    }
    args.out.with_suffix(".receipt.json").write_text(json.dumps(receipt, indent=2))
    print(f"PASS: {args.out} {size[0]}x{size[1]} in {seconds}s "
          f"(receipt: {args.out.with_suffix('.receipt.json')})")
    return 0


if __name__ == "__main__":
    sys.exit(main())
