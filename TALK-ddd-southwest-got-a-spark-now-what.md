...
2. **Agent executes the pinned steps** — build/serve the **llama-swap `:9000`** fleet (act one). Let it move; talk over it.
...
5. **Act two (optional): bolt on the front door.** Run the second runbook; **LiteLLM `:4000`** comes up over the green fleet and its **no-cloud-fallback gate** fires — the "one community feature I deliberately disable" beat. This is where the box becomes the full community stack (`client → LiteLLM → llama-swap → engines`). Skip it if time is tight — the fleet is a complete endpoint without it.
...
- **Honest about the stack (and why it's two acts):** the box comes up in two runbooks. **Act one** is the **llama-swap `:9000`** fleet — a complete, useful endpoint on its own. **Act two** (an optional second runbook) bolts the **LiteLLM `:4000`** control plane on top — the same martinB78/community stack, *built on*, not stripped down — for anyone who wants per-agent keys + spend tracking. I split them deliberately: a gateway is power not every box needs, and the seam keeps each runbook a clean, self-contained spec. The one community feature I deliberately **disable** (in act two): auto cloud-fallback — a gate that keeps the unattended path local (DECISION-DF-001, spec'd in DF-005). Direct `:9000` stays the documented LiteLLM-down fallback.
